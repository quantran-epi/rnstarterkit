...
import { signInUsernamePassword, signInUsernamePasswordFailed, signInUsernamePasswordSuccess } from '../reducers/AuthReducer';
    ...
    = (action$: Observable<IAction>, state$: StateObservable<RootState>, { Services }) => {
        return action$.pipe(
            ...
            switchMap(action => from(Services.Auth.SignInWithUsernamePassword(action.payload.Username, action.payload.Password)).pipe(
                map(user => signInUsernamePasswordSuccess({ user: user })),
                catchError(error => {
                    Logger.error(error);
